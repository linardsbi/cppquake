...
void M_Setup_Draw() {
    ...
    if (setup_cursor == 0)
        M_DrawCharacter(168 + 8 * setup_hostname.length(), setup_cursor_table[setup_cursor],
                        10 + ((int) (realtime * 4) & 1));
...
void M_AdjustSliders(int dir) {
    ...
    switch (options_cursor) {
        ...
        case 11:    // lookstrafe
            ...
            break;
        case 12:    // mouselook
            Cvar_SetValue("mouselook", !mouselook.value);
            break;
...
void M_Options_Draw() {
    ...
    M_DrawCheckbox(220, 128, mouselook.value);
...
void M_Options_Key(int k) {
    ...
    if (options_cursor == 13 && vid_menudrawfn == nullptr) {
        if (k == K_UPARROW)
            options_cursor = 12;
        else
            options_cursor = 0;
    ...
    switch (key) {
        ...
        case K_ENTER:
            ...
            if (modemConfig_cursor == 4) {
                (*SetModemConfig)(0, va("{:c}", modemConfig_dialing).data(), modemConfig_clear, modemConfig_init,
                                  modemConfig_hangup);
                m_entersound = true;
                M_Menu_SerialConfig_f();
            }
